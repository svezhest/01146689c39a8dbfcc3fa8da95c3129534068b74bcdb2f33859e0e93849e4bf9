...
companies = {
    ...
    2: (['мкб', 'cbom'], ['московск кредитн банк', 'credit bank']),
    3: (['росдорбанк', 'rdrb', 'рдбанк', 'rosdorbank'],
        ['российск акционерн коммерческ дорожн банк', 'дорожн банк', 'roads bank']),
    4: (['alrosa', 'alrs', 'алрос'], []),
    5: (['авангард', 'avan', 'avangard'], []),
    6: (['prmb', 'primbank', 'primorye', 'примор'], []),
    7: (['втб', 'vtbr', 'vtb'], []),
    ...
    11: (['rual', 'руса', 'rusal'], []),
    12: (['enpl', 'enpg'], ['эн груп', 'en group']),
    13: (['белон', 'blng', 'belon'], []),
    14: (['dzrd', 'дзрд'], ['донск завод радиодетал']),
    15: (['левенгук', 'lvhk', 'levenhuk'], []),
    16: (['lpsb', 'лэск', 'lpsbg', 'lesk'], ['липецк энергосбытов компан',
                                             'липецк эск']),
    17: (['mrku'], ['межрегиональн распределительн сетев компан ура',
                    'мрск ура', 'россет ура', 'rosseti ural']),
    18: (['multisistema', 'мультисистем', 'msst'], []),
    19: (['npof', 'npofizika'], ['научнопроизводствен объединен физик',
                                 'нпо физик']),
    20: (['mgnzg', 'mgnz'], ['соликамск магниев завод', 'оа смз']),
    21: (['инарктр', 'aqua', 'инарктик', 'inarctica'], ['русск аквакультур',
                                                        'русск мор']),
    22: (['abraudurso', 'abrd', 'абра'], ['абра  дюрс']),
    23: (['utar'], []),
    24: (['akrn', 'acron', 'акрон'], []),
    25: (['bane'], []),
    26: (['afks'], ['афк систем']),
    ...
    42: (['ogkb'], []),
    43: (['vsyd'], []),
    44: (['gaza'], ['групп газ', 'па газ']),
    45: (['газкона', 'газкон', 'gazc'], []),
    46: (['rostovoblgaz', 'rtgz', 'ростовоблгаз'],
         ['па газпр газораспределен ростовнадон', 'газпр рнд']),
    47: (['sibn'], ['газпр нефт', 'gazprom neft']),
    48: (['газпр', 'gazprom', 'gazp', 'газик'], ['па газпр']),
    ...
    53: (['nornickel', 'норникел', 'gmkn'],
         ['па горнометаллургическ компан норильск никел',
          'па гмк норильск никел', 'норильск никел']),
    54: (['grnt', 'гит'], ['па городск инновацион технолог',
                           'па гит', 'городск инновацион технолог']),
    55: (['rlmn', 'роллма', 'rollman'], []),
    56: (['smlt', 'samolet'], ['групп самолет']),
    57: (['лср', 'lsrg'], []),
    ...
    73: (['клсб', 'кск', 'klsb'], ['калужск сбытов компан',
                                   'калужск сбыткомп', 'kalujskaya sbytovaya']),
    74: (['kamaz', 'kmaz', 'камаз'], []),
    ...
    79: (['всмпоавсм', 'всмпо', 'всмпоависм', 'vsmpoavisma', 'vsmo'],
         ['корпорац всмпоависм', 'vsmpoavisma corporation']),
    ...
    87: (['kgkc', 'кгк', 'kgk'], ['курганск генерирующ компан',
                                  'курганск гк', 'па кгк', 'kurgancskaya generiruyushaya']),
    88: (['мвиде', 'mvideo', 'mvid'], []),
    89: (['mgnt', 'magnit'], ['па магн', 'магн па']),
    90: (['магнитк', 'ммк', 'magn'], ['magnitigorskiy metallurgicheskiy',
                                      'магнитогорск металлургическ комбинат']),
    ...
    118: (['нмтп', 'nmtp'], ['новороссийск морск порт',
                             'новороссийск морск торгов порт']),
    ...
    161: (['krkop'], ['ткз красн котельщик', 'ткз красн котельщик',
                      'красн котельщик', 'pjsc krasny kotelshchik', 'krasny kotelshchik']),
    ...
    178: (['тза', 'tuza'], ['па тза', 'па тза',
                            'туймазинск завод автобетоновоз', 'не tza  ест американск тикер']),
    ...
    189: (['wtcm', 'цмт', 'wtc'],
          ['центр международн торговл', 'world trade center']),
    190: (['trcn', 'трансконтейнер', 'transcontainer'], ['па трансконейнер']),
    191: (['cntl'], ['центральн телеграф',
                     'central telegraph', 'па центральн телеграф']),
    192: (['чзпснпрофнаст', 'prfn'],
          ['па чзпснпрофнаст', 'челябинск завод профилирова стальн наст']),
    193: (['чкпз', 'ckpz'],
          ['oao chelyabinsk forgeandpress plant', 'па челябинск кузнечнопрессов завод']),
    194: (['chmk', 'чмк'],
          ['chelyabinsk metallurgical plant', 'па чмк', 'челябинск металлургическ комбинат']),
    195: (['чтпз', 'chep'],
          ['chelyabinsk pipe rolling plant', 'па чтпз', 'челябинск трубопрокатн завод']),
    196: (['электроцинк', 'electrozink', 'eltz'], ['па электроцинк']),
    197: (['elfv', 'enru'], ['энел росс', 'enel russia', 'па энел росс']),
    198: (['unkl', 'unickel', 'южуралникел'],
          ['комбинат южуралникел', 'па комбинат южуралникел']),
    199: (['unipro', 'юнипр', 'upro'], ['па юнипр']),
    200: (['ятэк', 'yakg', 'yatec'],
          ['якутск топливноэнергетическ компан', 'па якутск топливноэнергетическ компан']),
    ...
    206: (['diod', 'диод'],
          ['diod plant of ecological technics and ecomeals',
           'па диод', 'завод экологическ техник и экопитан диод']),
    207: (['mobb'], []),
    208: (['nauk'], []),
    209: (['rnft'], []),
    210: (['omzz', 'omzzp'], []),
    ...
    218: (['etln'], ['etln li', 'эталон груп', 'групп эталон']),
    219: (['fixprice', 'fixp', 'фикспрайс'], ['fix price']),
    220: (['globaltrans', 'глобалтранс'], ['gltr li']),
    221: (['hhru', 'headhunter'], []),
    222: (['hmsg'], []),
    223: (['vkco', 'вк', 'вконтакт', 'vk'], ['mail li', 'vk group']),
    224: (['mdmg'], ['md medical']),
    225: (['tcsg', 'тинькофф', 'tinkoff', 'тинек', 'тинёк'], ['tcs li', 'tcs group']),
    226: (['okey'], []),
    227: (['five'], ['five li', 'х групп', 'x group']),
    228: (['qiwi'], ['а кив']),
    229: (['lnta', 'lent'], []),
    230: (['ozon', 'озон'], []),
    ...
    273: (['delimobil', 'делимоб', 'deli'],
          ['каршеринг рус', 'па каршеринг русс', 'каршеринг русс  па']),
    274: (['dias', 'diasoft'], [])
}
...
class WordScore:
    def __init__(self,
                 super_negative_count: float,
                 negative_count: float,
                 neutral_count: float,
                 positive_count: float,
                 super_positive_count: float):
...
def make_features(text: str,
                  word_scores: dict[str, list[float]]) -> list[list[float]]:
    return [get_text_word_metrics(' '.join(
        filter(
            lambda w: True if w in word_scores else False,
            (transform_word(word) for word in filter(
                lambda w: len(w) > 0,
                re.split('[?!.,:; \n\t]', text))))), word_scores)]


def make_features_splitted(splitted: list[str],
                           word_scores: dict[str,list[float]]) -> list[list[float]]:
    return [get_text_word_metrics(
        ' '.join(filter(
            lambda w: True if w in word_scores else False,
            (transform_word(word) for word in splitted))), word_scores)]


word_scores = None
with open('word_scores_full.json') as f:
...
def score_texts(
    ...
    for text in messages:
        ...
        res.append([(int(company_id), float(y[0][0])) for company_id in set(mentioned_companies)])
    return res
